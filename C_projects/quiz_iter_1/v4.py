...
def generate_window(img_path, constellation_num, options_given, img_num, conf_buffer):
    ...
    def check_answer(button_num):
        ...
        conf_buffer.append(confidence)      ###Potentisl Breakpoint CHECK HERE IF NECESSARY
        threshold = compute_dynamic_threshold(conf_buffer)  #Dynamically calculates threshold for current question
        correct = button_num == constellation_num
        confident = confidence >= threshold #Defines the person as confident if detection levels above threshold
